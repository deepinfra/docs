...
<Warning>
  Only set `Content-Encoding: gzip` if the body is **actually** gzip-compressed. Setting the
  header on an uncompressed body (a common mix-up with charset headers like `utf-8`) will make
  the request fail. `Content-Encoding` describes compression, not character set — charset
  belongs in `Content-Type` (e.g. `application/json; charset=utf-8`).
</Warning>
