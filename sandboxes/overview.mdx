...
If you don't pass `plan`, you get `medium` — not the cheapest tier. Disk scales with plan and is capped at 40 GB for the top tiers.

| Plan | vCPU | RAM | Disk | Price |
|------|------|-----|------|-------|
| `nano` | 1 | 1 GB | 5 GB | $0.054/hour |
| `small` | 2 | 2 GB | 10 GB | $0.107/hour |
| `medium` (default) | 2 | 4 GB | 20 GB | $0.134/hour |
| `large` | 4 | 8 GB | 40 GB | $0.268/hour |
| `xlarge` | 4 | 16 GB | 40 GB | $0.375/hour |
| `2xlarge` | 8 | 16 GB | 40 GB | $0.536/hour |
| `4xlarge` | 8 | 32 GB | 40 GB | $0.750/hour |

Rates can change — call the [catalog endpoint](#http-api) or `Sandbox.catalog()` to get current specs and pricing programmatically instead of hardcoding these numbers, or see the same table in the [dashboard](https://deepinfra.com/dash/sandboxes#catalog).
...
- **Disk** — scales with plan, capped at 40 GB (see [Plans and pricing](#plans-and-pricing)); the container's own disk outside `/workspace` is only a couple GB regardless of plan.
